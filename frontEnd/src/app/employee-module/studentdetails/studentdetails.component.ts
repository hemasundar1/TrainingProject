...
export class StudentdetailsComponent implements OnInit {
  ...
  public approve(x:any)
  {
    ...
    this.empserve.approveStatus(x.applicationDetails.appId,x).subscribe(data=>this.x=data);
    window.alert("REQUEST APPROVED !!!");
    this.route.navigate(['student/requests']);
  }
  public reject(x:any)
  {
    this.empserve.rejectStatus(x.applicationDetails.appId,x).subscribe(data=>this.x=data);
